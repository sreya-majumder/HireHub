...
//             {session ? (
//               <>
//               <p className="text-gray-300 hover:text-white px-4"><Link href={`/applicants/public-profile/${stored}`}>Profile</Link></p>
//               <button
//                 className="button1"
//                 onClick={() => {
//                   signOut();
...
export default function ApplicantProfile({
  ...
  useEffect(() => {
    const fetchData = async () => {
      const userInformation = session?.user as { _id?: string };
      const userId = userInformation?._id;
          ...
          <nav className="flex items-center justify-between py-4">
            ...
            <div className="hidden  md:flex md:items-center md:ml-auto md:mr--10 ">
              ...
              {session ? (
                <>
                  <p className="text-gray-300 hover:text-white px-4">
                    <Link href={`/applicants/public-profile/${stored}`}>
                      Profile
                    </Link>
                  </p>
                  <button
